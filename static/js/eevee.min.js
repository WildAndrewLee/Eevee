...
FieldTemplate.prototype.addValidator = function(a) {
    ...
    return this.$editor = $("<section>").addClass("editor").append($("<nav>").append($("<span>").addClass("main-button selected").text("Main")).append($("<span>").addClass("met-button").text("Met")).append($("<span>").addClass("stats-button").text("Stats")).append($("<span>").addClass("moves-button").text("Moves")).append($("<span>").addClass("ribbons-button").text("Ribbons")).append($("<span>").addClass("misc-button").text("Misc")).append($("<span>").addClass("choose-again-button").text("Start Over").click(function() {
        a.$editor.remove(), $("#thumbnails").show();
    })).append($("<span>").addClass("save-button").text("Save"))).append($("<section>").addClass("fields main").append($("<aside>").addClass("row").append($("<figure>").append(this.$thumbnail).append(this.$typing)).append(h))), 
    this.$editor;
}, $(function() {
    function a(a) {
        return new Promise(function(b, c) {
    ...
    function b() {
        return new Promise(function(b, c) {
            for (var d = [], e = 1; 493 >= e; e++) d.push(a("/img/pokemon/" + e + ".png"));
            Promise.all(d).then(function(a) {
                a.forEach(function(a, b) {
                    $("#thumbnails").append($("<figure>").append($(a).attr("id", "number-" + (b + 1))));
                }), b();
            }, c);
        });
    }
    b().then(API.prepare).then(function() {
        $("#loading").hide(), $("#thumbnails img").click(function() {
            var a = parseInt(this.id.split("-")[1]), b = new Pokemon(a);
            b.load().then(function() {
                $("#thumbnails").hide(), $("body").append(b.render());
            });
        });
    });
});